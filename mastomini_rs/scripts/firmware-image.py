...
Produces, next to the ELF:
  mastomini-esp32.bin             application image (0x10000)
  mastomini-partitions.bin        partition table from partitions.csv (0x8000)
  mastomini-bootloader.bin        second-stage bootloader (0x0), first install only
...
def main():
    ...
    table = out / 'mastomini-partitions.bin'
    ...
    shutil.copyfile(candidates[-1], out / 'mastomini-bootloader.bin')
    print(f'Application: {image} ({size} / {APP_LIMIT} bytes)')
    print(f'Partition table: {table}')
    print(f'Bootloader: {out / "mastomini-bootloader.bin"}')
