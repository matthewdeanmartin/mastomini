...
// app, and `npm start` proxies /api to a server (proxy.conf.mjs).
//
...
const TOKEN_KEY = 'mastobots.token';
...
export class Api {
  ...
  private async request<T>(method: string, path: string, body?: Body): Promise<T> {
    ...
    try {
      ...
      throw new ApiError(0, 'Could not reach the server. Is the bots board on, and are you on the home Wi-Fi?');
    }
