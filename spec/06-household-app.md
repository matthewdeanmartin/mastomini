...
| `GET /me/devices`, `DELETE /me/devices/:id` | member | **done.** Newest first: `id`, `app {name, website}`, `scopes`, `created_at`, `last_used_at` (since boot, else `null`), `current`. API keys are not listed; page sign-ins are, as `Web browser` |
| `GET /me/api_keys`, `POST /me/api_keys`, `DELETE /me/api_keys/:id` | member | **done.** Long-lived tokens for bots (`domain/local_tokens.rs`): a `TokenRec` with `app_id` 0 plus a `LocalTokenRec` (kind, name). POST needs `name`, `password`, optional `scopes` (default `read write`) and returns `key` once. 4 per member, 16 overall, outside session eviction; the token epoch (password change, sign out everywhere) ends them; no DM seal. Posts name the key as `application` |
| `GET/POST /web/signin`, `POST /web/signout` | anyone | **done.** Browser session for the post/profile pages (`api/pages.rs`): read-only local token in cookie `mm_session` (HttpOnly, SameSite=Lax, Secure over HTTPS, 30 days). Pages only; the API never reads cookies. `Origin` must match `Host` on POST; `return` must be a local path |
...
| `GET /version` | public | **done.** Build identity: package version, `fingerprint` of all firmware inputs (build.rs), git `commit`/`dirty`, `built_at`, profile, features, target, uptime. Nothing household-specific. `scripts/firmware-version.py` (`make board-version`) compares it with the working tree. `/diag` includes the same `build` object |
