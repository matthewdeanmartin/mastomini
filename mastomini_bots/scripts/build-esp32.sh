...
if [[ -d /c/Espressif/frameworks/esp-idf-v5.5.3 ]]; then
  ...
  # (C:/mmb: separate from mastomini's C:/mmr, so both can build at once).
  export CARGO_TARGET_DIR="${CARGO_TARGET_DIR:-C:/mmb}"
...
# The firmware embeds the admin site certificate, from mastomini's household
# CA: create it if missing (never
# replaces one), and check it either way (README.md).
bash scripts/certs.sh
# The firmware carries the admin app (feature bundled-web).
bash scripts/build-web.sh
cargo +esp build --locked --release --no-default-features --features esp32 \
  --bin mastomini-bots-esp32 --target xtensa-esp32s3-espidf -Z build-std=std,panic_abort "$@"
...
"$esp_python" scripts/firmware-image.py "${CARGO_TARGET_DIR:-target}/xtensa-esp32s3-espidf/release/mastomini-bots-esp32"
